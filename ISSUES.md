...
| 41 | **The utility tools demanded an NVIDIA key even with another provider configured.** `generate_training_data`, `evaluate_with_llm`, `test_model`, `generate_script`, and `compare_models` were all hardcoded to NVIDIA NIM. A user with a working Ollama key, in a live session already talking to kimi-k2.6, was told "synthetic data generation needs an NVIDIA API key" and the run stalled | `chat/tools.py`, 6 call sites | **Fixed** — new `chat/utility_llm.py` resolves whichever OpenAI-compatible provider is configured (Ollama cloud or local → NVIDIA → OpenAI). Verified generating Alpaca pairs through kimi-k2.6 with no NVIDIA key |
| 42 | `generate_embeddings` gave the same generic NVIDIA error, hiding that it is the one tool with no alternative — Ollama Cloud returns 404 for `/v1/embeddings` | `chat/tools.py` | **Fixed** — says so explicitly, and points at local Ollama as the other option |
| 43 | `check_training_status` greps the host's whole `ps aux` for any line containing both "llm_forge" and "train", so unrelated processes were reported as "training in progress". Failed once mid-suite for exactly this reason | `chat/tools.py:1488` | **Fixed** — matches actual training invocations; test stubbed so it no longer depends on what else is running |
